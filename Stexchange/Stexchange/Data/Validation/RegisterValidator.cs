...
namespace Stexchange.Data.Validation
{
    public class RegisterValidator : AbstractValidator<RegisterModel>
    {
        public RegisterValidator()
        {
            ...
            RuleFor(x => x.Postalcode).Must(x => x.Length == 6).WithMessage("Ongeldige postcode");
    ...
    public class ChangeAccountSettingsValidator : AbstractValidator<AccountSettingsModel>
    {
        public ChangeAccountSettingsValidator()
        {
            ...
            RuleFor(x => x.Postalcode).Must(x => x.Length == 6).WithMessage("Ongeldige postcode");
            RuleFor(x => x.Postalcode).Must(x => new Regex(@"\d{4}[A-Z]{2}", RegexOptions.IgnoreCase).IsMatch(x)).WithMessage("Ongeldige postcodeTest").When(x => x.Postalcode.Length > 0);
